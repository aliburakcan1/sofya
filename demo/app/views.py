...
class DefaultFormView(FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        def saveFig(list_of_dict, isim, col_width):
        ...
        for tweet in tweepy.Cursor(api.user_timeline, id=request.POST.get('username'),
        ...
        saveFig(sorted(list_of_dicts, key=lambda k: k['count'], reverse=True)[:100], 'table1.png', 2.0)
        saveFig(sorted(list_of_dicts2, key=lambda k: k['count'], reverse=True)[:100], 'table2.png', 3.0)
        saveFig(sorted(list_of_dicts3, key=lambda k: k['count'], reverse=True)[:100], 'table3.png', 4.0)
        yuzde = 0
